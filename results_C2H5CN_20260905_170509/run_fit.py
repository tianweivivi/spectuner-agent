#!/usr/bin/env python3
"""Spectuner C2H5CN identification for IRAS 16293-2422 B."""
...
def build_config():
    ...
    config.set_ident_species(species=["C2H5CN"], collect_iso=True,
...
def run_fit(config):
    start = time.time()
    print("\nPhase 1: Individual line identification (C2H5CN)")
...
def generate_summary(config, total, t1, t2):
    ...
    with open(path, "w") as f:
        f.write(f"# Spectuner C2H5CN Identification Summary\n# Generated: {ts}\n")
...
def plot_results(config, freq_ranges):
    ...
    try:
        ...
        fig.suptitle(f"IRAS 16293-2422 B — C2H5CN Family Overview (ALMA Band 6)", fontsize=12, y=0.998)
...
def main():
    ...
    print(f"Target: C2H5CN (ethyl cyanide / propionitrile)")
